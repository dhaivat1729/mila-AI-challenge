...
__all__ = ['fcn_resnet50', 'fcn_resnet101', 'deeplabv3_resnet50', 'deeplabv3_resnet101',
           ...
           'deeplabv3_mobilenet_v3_small', 'lraspp_mobilenet_v3_small']
...
def _segm_model(name, backbone_name, num_classes, aux, pretrained_backbone=False):
...
def _load_model(arch_type, backbone, pretrained, progress, num_classes, aux_loss, **kwargs):
    ...
    if pretrained:
        ...
        ## since the model is not pretrained
        ## we initialize weights and biases 
        for name, p in model.named_parameters():
            if 'classifier' in name:
                if 'bias' in name:
                    torch.nn.init.constant_(p.data, 1.0)
                elif 'weight' in name:
...
def _load_weights(model, arch_type, backbone, progress):
    ...
    if model_url is None:
        ...
        for key, value in model_dict.items():
            ...
            if key == state_dict_list[count][0] and state_dict_list[count][1].size() == value.size():
                model_dict[key] = value
            else:
                if 'bias' in key:
                    torch.nn.init.constant_(value, 0.5)
                elif 'weight' in key:
                    torch.nn.init.normal_(value)
                model_dict[key] = value
                # print(key, state_dict_list[count][0])
            count += 1
...
"""
    For these models, only backbone weights are loaded! Rest are random initialization!
"""

def deeplabv3_mobilenet_v3_small(pretrained=False, progress=True,
                                 num_classes=21, aux_loss=None, **kwargs):
    """Constructs a DeepLabV3 model with a MobileNetV3-small backbone.

    Args:
        pretrained (bool): Only backbone is loaded
        progress (bool): If True, displays a progress bar of the download to stderr
        num_classes (int): number of output classes of the model (including the background)
        aux_loss (bool): If True, it uses an auxiliary loss
    """
    return _load_model('deeplabv3', 'mobilenet_v3_small', pretrained, progress, num_classes, aux_loss, **kwargs)

def lraspp_mobilenet_v3_small(pretrained=False, progress=True,
                                 num_classes=21, aux_loss=None, **kwargs):
    """Constructs a Lite R-ASPP model with a MobileNetV3-small backbone.
